...
const CreateProductModal = ({ isOpen, onClose, onSave }) => {
  ...
  // Vincular nombre de categoría con su id
  useEffect(() => {
    const found = categories.find(
      (c) => c.name.trim().toLowerCase() === categoryInput.trim().toLowerCase()
    );
  ...
  // Post creación → subida de archivos opcional
  useEffect(() => {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {showSuccess && (
          <div className="fixed top-20 right-5 z-[10000]">
            <SuccessMessage message="¡Producto creado exitosamente!" />
          </div>
        )}
