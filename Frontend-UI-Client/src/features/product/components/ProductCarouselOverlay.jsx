...
const ProductCarouselOverlay = ({
    images = [],
    productId,           // se mantiene por compatibilidad (products)
    onClose,
    onDeleteRequest,
    editable = false,
    isEmbedded = false,
    ...
    const getMediaType = (type = "", filename = "") => {
        ...
        if (type?.startsWith("video/") || ["mp4", "webm", "mov", "avi", "mkv", "mpeg"].includes(ext))
            return "video";
    ...
    useEffect(() => {
        ...
        const formatted = images
            .map((img) => ({
                id: img.drive_file_id || img.id || img.key,
                filename: img.filename || img.name || img.key || "archivo",
                contentType: img.contentType || img.content_type || img.mime_type || img.mimeType || "application/octet-stream",
                url: img.url, // URL presignada o blob URL
            }))
            .filter((i) => i.url);
    ...
    return (
        <div className="w-full">
            <div className="relative w-full pt-[66%] bg-background-100 rounded overflow-hidden mb-4">
                {/* Contenido */}
            ...
            {/* Indicadores */}
            <div className="flex justify-center py-2 border-t rounded">
                {localImages.map((_, i) => (
                    <button
                        key={i}
                        onClick={() => {
