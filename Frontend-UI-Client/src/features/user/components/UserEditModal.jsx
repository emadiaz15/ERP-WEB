...
const UserEditModal = ({
  ...
  // Obtener usuario actual desde localStorage o contexto
  const currentUser = JSON.parse(localStorage.getItem('user') || '{}');
  const isAdmin = currentUser?.is_staff === true;

  const handleDeleteImage = () => {
    console.log("Intentando borrar imagen:", user.image, "de user_id:", user.id);
    if (!isAdmin) {
      setInternalError('Solo un usuario admin puede eliminar imágenes de perfil.');
      return;
    }
    if (!user.image || user.image === '') {
      setInternalError('No hay imagen para eliminar.');
      return;
    }
    openDeleteConfirmModal({
      type: 'delete-image',
              ...
              {(formData.image || hasImage) ? (
                <div className="flex items-center space-x-2 max-w-[220px] truncate">
                  ...
                  <button
                    ...
                    disabled={!user.image || user.image === ''}
                  >
