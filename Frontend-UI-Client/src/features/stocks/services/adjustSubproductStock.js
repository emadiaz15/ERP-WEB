...
/**
 * Ajusta el stock de un subproducto y devuelve el evento mapeado para la UI.
 * @param {number|string} subproductId
 * @param {object} values
 */
export async function adjustSubproductStock(subproductId, values) {
    try {
        const { data } = await djangoApi.post(`/stocks/subproducts/${subproductId}/stock/adjust/`, values);
